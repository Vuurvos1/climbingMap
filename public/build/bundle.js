...
var app = (function () {
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			t = space();
    			ul = element("ul");
    			attr_dev(div, "class", "svgContainer svelte-tu8w8v");
    			add_location(div, file, 129, 2, 3099);
    			add_location(ul, file, 142, 2, 3390);
    			add_location(main, file, 128, 0, 3090);
    		},
    		...
    		p: function update(ctx, [dirty]) {
    			if (dirty & /*svg*/ 1) div.innerHTML = /*svg*/ ctx[0];		},
    		i: noop$4,
    		o: noop$4,
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	async function fetchData() {
    		const filters = { filters: { deleted: false, live: true } };
    		const url = `${baseUrl}/gyms/8/climbs?json_params=${encodeURIComponent(JSON.stringify(filters))}`;
    	...
    	async function fetchSvg() {
    		const url = 'https://cdn1.toplogger.nu/images/gyms/bruut_boulder_breda/floorplan.svg';
    		const res = await fetch(url);
    	...
    	function d3ify() {
    		...
    		const svg = select('svg.flex');

    		const zoom$1 = zoom().scaleExtent([0.1, 2.5]).on('zoom', e => {
    			const el = select('g#zoom_layer');
    			el.attr('transform', e.transform);
    		});

    		svg.call(zoom$1);

    		select('g#zoom_layer').on('click', (e, d) => {
    			e.stopPropagation();
    			...
    			svg.transition().duration(750).call(zoom$1.transform, identity.translate(elw / 2, elh / 2).scale(Math.min(8, 0.9 / Math.max(elw / w, elh / h))).translate(-(box.left + box.right) / 2, -(box.top + box.bottom) / 2));
    		});
    	} // svg.call(
    	//   zoom.transform,
    	...
    	$$self.$inject_state = $$props => {
    		if ('climbs' in $$props) climbs = $$props.climbs;
